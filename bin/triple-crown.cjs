...
const CAPABILITIES = ['triple-superpowers', 'triple-gstack', 'triple-crown-guide'];
...
const SKILL_MARKER = '.triple-crown-skill';
...
function run(cmd,args,opts={}) {
  ...
  if (r.code!==0) {
    ...
    if(/runtimeCompat\.(supported|notes) references unknown runtime/i.test(tail)) {
      extra='\nTriple Crown hint: this is a GSD concrete-runtime cross-capability validation failure. v0.6.3+ bundles use runtimeCompat ["*"] specifically to avoid this coupling.';
    }
...
function resolveGsd(root) {
  if(process.env.TRIPLE_GSD_BIN) {
    const p=path.resolve(process.env.TRIPLE_GSD_BIN);
...
function detectGstack() {
  const source=process.env.TRIPLE_GSTACK_HOME
    ? path.resolve(process.env.TRIPLE_GSTACK_HOME)
...
function findSuperpowers() {
  const explicit=process.env.TRIPLE_SUPERPOWERS_HOME;
...
function prepareStableSource(root) {
  const dest=path.join(root,'.triple-crown');
  const tmp=path.join(root,`.triple-crown.tmp-${process.pid}`);
  const backup=path.join(root,`.triple-crown.backup-${process.pid}`);
...
function installCapabilities(root,runner,opts) {
  ...
  for(const id of CAPABILITIES) {
    ...
    const spec=`./.triple-crown/capabilities/${id}`;
...
// active/surfaced while nothing ever reaches Claude Code. Triple Crown keeps the
...
function installProjectSkills(root) {
  ...
  const installed=[];
  for(const id of CAPABILITIES) {
    const srcRoot=path.join(root,'.triple-crown','capabilities',id,'skills');
    if(!exists(srcRoot)) continue;
    for(const stem of capabilitySkillStems(path.join(root,'.triple-crown'),id)) {
...
function removeShipGuard(root) {
  const hook=path.join(root,'.claude','hooks','triple-crown-ship-guard.cjs');
...
function doctor(root,opts={}) {
  ...
  add('gstack-skill-id',
    ...
    gstack.shortSkills?'short skills active (/review)':(gstack.prefixedSkills?'only namespaced gstack-review detected; Triple Crown defaults expect short IDs':'review skill not surfaced'));
  ...
  add('stable-source',exists(path.join(root,'.triple-crown','VERSION'))?'PASS':'FAIL',path.join(root,'.triple-crown'));
  for(const id of CAPABILITIES) {
    const row=rows.find(x=>x.id===id && x.scope==='project');
  ...
  const hookPath=path.join(root,'.claude','hooks','triple-crown-ship-guard.cjs');
  ...
  for(const group of (settings&&settings.hooks&&Array.isArray(settings.hooks.PreToolUse)?settings.hooks.PreToolUse:[])) {
    for(const h of (Array.isArray(group&&group.hooks)?group.hooks:[])) {
      ...
      if(c.includes('triple-crown-ship-guard.cjs')) guardCommands.push(c);
    }
  }
...
function printDoctor(d,jsonMode) {
  ...
  log(`Triple Crown installer doctor — v${d.version}`);
...
function runStatus(root,args=[]) {
  const staged=path.join(root,'.gsd','capabilities','triple-crown-guide','checks','workflow-guide.cjs');
  const stable=path.join(root,'.triple-crown','capabilities','triple-crown-guide','checks','workflow-guide.cjs');
  const script=exists(staged)?staged:stable;
  if(!exists(script)) fail('Triple Crown guide is not installed. Run install first.');
...
async function install(root,opts) {
  ...
  const actions=[
    ...
    `install ${expectedSkillDirs().length} Triple Crown skills into ${path.join(root,'.claude','skills')}`,
  ];
  if(opts.routing) actions.push('add/update a managed Triple Crown section in CLAUDE.md');
  ...
  const nodeV=process.version.replace(/^v/,'');
  if(!gte(nodeV,[24,0,0]) && process.env.TRIPLE_CROWN_ALLOW_UNSUPPORTED_NODE !== '1') {
  ...
  if(!gstack.shortSkills) {
    ...
    if(gstack.prefixedSkills) warn('Namespaced gstack skills are present. Re-run gstack setup with --no-prefix or configure Triple Crown skill IDs.');
    else warn('Run gstack setup before using Triple Crown quality adapters.');
  ...
  try {
    ...
    tx.commit();
    log('');
    log(`Triple Crown v${VERSION} installed successfully.`);
    log(`Project: ${root}`);
    ...
    log('Next: restart/reload Claude Code if needed, then run /gsd-triple-crown');
    log('Doctor: npx triple-crown-workflow-installer doctor');
  } catch(err) {
    tx.restore();
    throw err;
  }
...
async function uninstall(root,opts) {
  const actions=[
    'remove Triple Crown project capability registrations',
    'remove .triple-crown managed source directory',
    'remove Triple Crown skills from .claude/skills',
    'remove managed CLAUDE.md routing block',
    'remove Triple Crown ship guard hook',
  ];
  ...
  fs.rmSync(path.join(root,'.triple-crown'),{recursive:true,force:true});
  removeRouting(root);
  removeShipGuard(root);
  if(removedSkills.length) log(`Removed skills: ${removedSkills.join(', ')}`);
  log(`Triple Crown project integration removed from ${root}`);
}
function help() {
  log(`Triple Crown Workflow Installer v${VERSION}

Usage:
  triple-crown [install] [options]
  triple-crown doctor [--project PATH] [--json]
  triple-crown status [--project PATH]
  triple-crown uninstall [--project PATH] [--yes]
  triple-crown version
...
main().catch(err=>{
  process.stderr.write(`Triple Crown installer: ${err.message}\n`);
