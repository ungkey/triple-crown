...
def main():
    ...
    try:
        ...
        hook=project/".claude"/"hooks"/"crew-ship-guard.cjs"
        assert hook.exists()
        ...
        commands=[h["command"] for g in settings["hooks"]["PreToolUse"] for h in g.get("hooks",[])
                  if "crew-ship-guard.cjs" in h.get("command","")]
        ...
        for name in sorted(expected):
            ...
            assert (skills/name/".crew-skill").exists(),f"{name}: ownership marker missing"
        ...
        matching=[g for g in groups if any("crew-ship-guard.cjs" in h.get("command","") for h in g.get("hooks",[]))]
        ...
        assert "Crew Status" in st.stdout
        ...
        assert not (project/".claude"/"hooks"/"crew-ship-guard.cjs").exists()
        assert not (project/".claude"/"skills"/"crew-gsd").exists()
        assert (keep/"SKILL.md").exists(),"uninstall deleted an unmanaged skill"
        assert not (project/"CLAUDE.md").exists() or "crew:managed-routing" not in (project/"CLAUDE.md").read_text()
