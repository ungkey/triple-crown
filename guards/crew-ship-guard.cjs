...
process.stdin.on('end', () => {
  ...
  const tc = path.join(root, '.planning', '.crew');
  ...
  if (actions.includes('prMerge')) {
    deny('Crew: PR/MR merge is not authorized by the GSD ship owner. Merge remains an explicit external action.');
  }
  ...
  if (ship && ship.kind === 'gsd-ship' && !expired(ship)) {
    if (ship.branch) {
      ...
      if (branch !== ship.branch) {
        deny(`Crew: armed GSD ship branch is ${ship.branch}, current branch is ${branch || '(detached)'}.`);
      }
    }
    for (const action of actions) {
      const r = bindAndConsume(shipPath, ship, sessionId, action);
      if (!r.ok) deny(`Crew ship guard: ${r.reason}`);
    }
    allow('Crew: protected remote effect authorized by active GSD ship.');
  }
  ...
  if (actions.length === 1 && actions[0] === 'gitPush' &&
    ...
    if (docs.boundSessionId && docs.boundSessionId !== sessionId) {
      deny('Crew: docs push authorization belongs to another Claude session.');
    }
    const files = listDiffFiles(root, docs.baseHead);
    if (files == null) deny('Crew: could not inspect document-release commit range; push denied.');
    const forbidden = files.filter(f => !pathAllowed(f, docs.allowedPaths));
    if (forbidden.length) {
      deny(`Crew: document-release push contains non-authorized paths: ${forbidden.join(', ')}. Re-run GSD verification/ship for runtime-affecting changes.`);
    }
    const r = bindAndConsume(docsPath, docs, sessionId, 'gitPush');
    if (!r.ok) deny(`Crew docs guard: ${r.reason}`);
    allow(`Crew: docs-only push authorized (${files.length} changed path(s)).`);
  }

  deny(
    'Crew: GSD owns ship in this project. Run the GSD ship workflow; ' +
