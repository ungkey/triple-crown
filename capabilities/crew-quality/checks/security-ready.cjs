...
function fail(c,m){console.error(`Crew security ship gate: ${m}`);process.exit(c);}
try{
 ...
 const th=s.blockOn||'high'; if(th==='none'){console.log(`Crew security-ready: PASS (${s.status}, advisory only)`);process.exit(0);}
 ...
 console.log(`Crew security-ready: PASS (${s.status}; 0 open findings >= ${th})`);
