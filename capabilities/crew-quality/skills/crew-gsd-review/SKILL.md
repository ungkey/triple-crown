...
2. Capture the Crew pre-review snapshot BEFORE invoking gstack.
...
```bash
node "$CREW_CAP/checks/review-session.cjs" begin "$PHASE_DIR"
```
...
if command -v gsd-tools >/dev/null 2>&1; then
  REVIEW_SKILL_ID=$(gsd-tools query config-get crew.gstack.review_skill_id 2>/dev/null || echo "review")
else
...
```text
$PHASE_DIR/.crew/gstack-review-summary.md
```
...
```bash
node "$CREW_CAP/checks/review-session.cjs" finalize "$PHASE_DIR" \
  --status <pass|concerns|blocked|unavailable> \
  --summary-file "$PHASE_DIR/.crew/gstack-review-summary.md"
```
...
The mutation invalidated any earlier Crew evidence whose workspace digest
no longer matches.
...
```bash
node "$CREW_CAP/checks/lib/evidence-store.cjs" record "$PHASE_DIR" \
...
```bash
node "$CREW_CAP/checks/verify-ready.cjs" "$PHASE_DIR"
```
