...
description: Triple Crown situational dashboard — show current progress, blockers, next action, help, recovery guidance, or integration health without taking lifecycle ownership from GSD.
...
<objective>
Keep the user oriented while a Triple Crown project moves through GSD + gstack +
Superpowers.

This is a **read-only navigation skill**. It may inspect `.planning`, Triple Crown
...
or `/gsd-ship`. Recommend a Triple Crown adapter command only when the missing
checkpoint is specifically one of Triple Crown's external quality/release adapters.
</objective>
...
- **doctor** — project-level Triple Crown installation/guard health. This is not the v0.6 host compatibility doctor.
</modes>

<process>
1. Resolve the capability directory:
   - project: `.gsd/capabilities/triple-crown-guide`
   - global: `$HOME/.gsd/capabilities/triple-crown-guide`
   - source-tree fallback: the directory containing this skill's capability
2. Invoke the deterministic guide:
   ```bash
   node "$TC_GUIDE_CAP/checks/workflow-guide.cjs" $ARGUMENTS
   ```
