...
function findGstackChecks(root) {
  if (process.env.TRIPLE_CROWN_GUIDE_DISABLE_PROBES === '1') return null;
  const candidates = [
    path.join(root,'.gsd','capabilities','triple-gstack','checks'),
    path.resolve(__dirname,'..','..','triple-gstack','checks'),
    path.join(os.homedir(),'.gsd','capabilities','triple-gstack','checks'),
  ];
...
function buildSnapshot(opts) {
  ...
  if (!root) {
    ...
    return {
      ...
      notes:['Triple Crown guide is read-only; GSD owns project initialization.']
    };
  }
  ...
  return {
    ...
    notes:[
      ...
      'Use /gsd-progress for GSD-native project progress; this dashboard adds Triple Crown quality/release checkpoints.'
    ]
  };
}
...
function determineNext(ctx) {
  ...
  if (stage('plan').state !== 'done') {
    ...
    return {
      ...
      after:[
        `node .gsd/capabilities/triple-gstack/checks/mark-plan-reviewed.cjs "${ctx.phase.dir}" --status pass`,
        '/gsd-progress --next'
      ]
    };
  }
  ...
  if (stage('review').state !== 'done') return {kind:'triple-crown-adapter',command:`/gsd-triple-gstack-code-review ${n}`,reason:'Independent gstack code review is missing or blocked.'};
  if (stage('evidence').state === 'blocked') return {kind:'recovery',command:`/gsd-triple-gstack-code-review ${n}`,reason:'Review/evidence freshness gate is blocked; re-review or record fresh post-review verification before GSD verify.'};
  if (stage('qa').state !== 'done') return {kind:'triple-crown-adapter',command:`/gsd-triple-gstack-qa-only ${n}`,reason:'Report-only browser/functional QA is missing or unusable.'};
  ...
  if (sec.state === 'blocked') return {kind:'recovery',command:`/gsd-triple-gstack-cso ${n}`,reason:'Security gate is blocked. Fix/resolve findings, then re-run external CSO and GSD verification as needed.'};
  if (sec.state !== 'done') return {kind:'triple-crown-adapter',command:`/gsd-triple-gstack-cso ${n}`,reason:'External CSO evidence is not current/complete; GSD native security remains independent.'};
  ...
  if (deploy.state === 'blocked') {
    return {kind:'release',command:`/gsd-triple-gstack-release-observe ${n}`,reason:'Recorded deployment does not match the effective GSD release. Refresh PR/deployment evidence with the actual deployed SHA.'};
  }
  if (deploy.state !== 'done') {
    return {
      kind:'release',
      command:`/gsd-triple-gstack-release-observe ${n}`,
      reason:'The GSD release exists, but matching deployment evidence is still required before Canary.',
      template:`/gsd-triple-gstack-release-observe ${n} --deployment-url <url> --deployed-sha <sha> --canary`
    };
  }
  ...
  if (can.state !== 'done') return {kind:'release',command:`/gsd-triple-gstack-release-observe ${n} --canary`,reason:'Matching deployment exists; Canary has not passed yet.'};
...
const HELP = {
  ...
  qa: [
    'QA',
    '  Triple Crown uses gstack /qa-only, not /qa.',
  ...
  verify: [
    ...
    '  Triple Crown verify:pre gates only check that external review/QA evidence is current.'
  ],
  gaps: [
    'Gap closure',
  ...
  ship: [
    ...
    '  gstack /ship is not part of a GSD-controlled Triple Crown lifecycle.'
  ],
  release: [
    'Release/deployment',
  ...
  recovery: [
    ...
    '  Run /gsd-triple-crown resume to see the last durable checkpoint and blocker.',
...
function helpText(topic) {
  if (!topic) {
    return [
      ...
      'Usage:',
      '  /gsd-triple-crown',
      '  /gsd-triple-crown status [--phase N]',
      '  /gsd-triple-crown next [--phase N]',
      '  /gsd-triple-crown resume [--phase N]',
...
function doctorSnapshot(opts) {
  ...
  add('triple-gstack', exists(path.join(root,'.gsd','capabilities','triple-gstack','capability.json')), 'project capability staged');
  add('triple-superpowers', exists(path.join(root,'.gsd','capabilities','triple-superpowers','capability.json')), 'project capability staged');
  add('triple-crown-guide', exists(path.join(root,'.gsd','capabilities','triple-crown-guide','capability.json')) || path.resolve(__dirname,'..').startsWith(root), 'guide capability staged/source');
  add('ship-guard', exists(path.join(root,'.claude','hooks','triple-crown-ship-guard.cjs')), '.claude hook installed', 'optional');
...
function ownerFor(kind) {
  ...
  if (kind === 'triple-crown-adapter') return 'gstack adapter under Triple Crown';
  if (kind === 'release') return 'GSD release evidence + Triple Crown adapter';
...
function printStatus(snap, compact=false) {
  const lines = [];
  lines.push('Triple Crown Status');
  lines.push('');
  if (!snap.projectRoot) {
  ...
  lines.push('');
  lines.push('Help: /gsd-triple-crown help <topic>');
...
function main() {
  ...
  if (opts.mode === 'doctor') {
    ...
    if (opts.json) console.log(JSON.stringify(d,null,2));
      ...
      console.log('Triple Crown Project Doctor\n');
...
try { main(); }
  ...
  console.error(`triple-crown-guide: ${err.message}`);
  process.exit(2);
}
