...
function tcDir(root) {
  const d = path.join(root, '.planning', '.crew');
...
function armGsd(phaseDir) {
  ...
  const ttl = configNumber('crew.ship.authorization_ttl_seconds', 300);
  ...
  console.log(`Crew ship owner armed: GSD (${branch}, expires ${auth.expiresAt})`);
}
...
function armDocs(phaseDir, allowVersion = false) {
  ...
  console.log(`Crew docs push armed (${branch}, VERSION=${allowVersion ? 'allowed' : 'denied'})`);
}
...
function disarmGsd(phaseDir) {
  ...
  console.log('Crew GSD ship authorization disarmed');
}
function disarmDocs(phaseDir) {
  const root = findProjectRoot(phaseDir);
  unlinkSafe(docsAuthPath(root));
  console.log('Crew docs push authorization disarmed');
}
function disarm(phaseDir) {
  const root = findProjectRoot(phaseDir);
  unlinkSafe(authPath(root));
  unlinkSafe(docsAuthPath(root));
  console.log('Crew ship authorization disarmed');
}
