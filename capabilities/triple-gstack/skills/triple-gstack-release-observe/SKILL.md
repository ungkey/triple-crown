---
name: gsd-triple-gstack-release-observe
...
```text
/gsd-triple-gstack-release-observe 3
/gsd-triple-gstack-release-observe 3 --deployment-url https://app.example.com
/gsd-triple-gstack-release-observe 3 --deployment-url https://app.example.com --deployed-sha abc123 --canary
/gsd-triple-gstack-release-observe 3 --retro
/gsd-triple-gstack-release-observe 3 --document
```
</arguments>
...
Resolve `TC_GSTACK_CAP` and `PHASE_DIR`.
...
```bash
node "$TC_GSTACK_CAP/checks/release-ledger.cjs" record-deployment "$PHASE_DIR" \
  --status deployed \
  --url "<url>" \
