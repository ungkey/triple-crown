---
name: gsd-triple-gstack-cso
description: Run a risk-based independent gstack CSO audit and normalize findings for the Triple Crown ship gate.
allowed-tools:
  - Bash
  - Read
  - Write
...
Resolve `TC_GSTACK_CAP` and `PHASE_DIR` as in the other Triple Crown wrappers.

## 2. Read Triple Crown security config
...
```bash
RISK_JSON=$(node "$TC_GSTACK_CAP/checks/security-risk.cjs" "$PHASE_DIR")
```
...
```bash
node "$TC_GSTACK_CAP/checks/security-session.cjs" begin "$PHASE_DIR"
```
...
```text
$PHASE_DIR/.triple-crown/security-normalized.json
```
...
```bash
node "$TC_GSTACK_CAP/checks/security-session.cjs" finalize \
  "$PHASE_DIR" \
  "$PHASE_DIR/.triple-crown/security-normalized.json"
```
