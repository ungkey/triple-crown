---
name: gsd-triple-gstack-qa-only
...
Resolve `TC_GSTACK_CAP` exactly as the code-review wrapper does, then:
...
```bash
node "$TC_GSTACK_CAP/checks/qa-session.cjs" begin "$PHASE_DIR"
```
...
if command -v gsd-tools >/dev/null 2>&1; then
  QA_SKILL_ID=$(gsd-tools query config-get triple_crown.gstack.qa_skill_id 2>/dev/null || echo "qa-only")
else
  QA_SKILL_ID="qa-only"
fi
```
...
```text
$PHASE_DIR/.triple-crown/qa-normalized.json
```
...
```bash
node "$TC_GSTACK_CAP/checks/qa-session.cjs" finalize \
  "$PHASE_DIR" \
  "$PHASE_DIR/.triple-crown/qa-normalized.json"
```
...
```bash
node "$TC_GSTACK_CAP/checks/uat-bridge.cjs" \
  "$PHASE_DIR" \
...
```bash
node "$TC_GSTACK_CAP/checks/qa-ready.cjs" "$PHASE_DIR"
```
