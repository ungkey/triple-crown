...
function fail(code, message, phaseDir) {
  console.error(`Triple Crown gstack plan-review gate: ${message}`);
  if (phaseDir) {
    console.error('');
    ...
    console.error(`     node .gsd/capabilities/triple-gstack/checks/mark-plan-reviewed.cjs ${JSON.stringify(phaseDir)} --status pass`);
